#!/usr/bin/env python3
"""
VEX CHART MARKUP - XAU/USD & XAG/USD Multi-Timeframe ICT Analysis
Gold and Silver — Full ICT Markup with Trade Thesis
"""
import os
import sys
sys.path.insert(0, '/Users/villain/Documents/transfer/ICT_WORK/ict_trainer/src')

import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
from datetime import datetime
import subprocess

from ict_agent.data.oanda_fetcher import OANDAFetcher

plt.style.use('dark_background')
fetcher = OANDAFetcher()

# Metal configs
METALS = {
    'XAU_USD': {
        'name': 'Gold (XAU/USD)',
        'pip_size': 0.10,       # $0.10 = 1 pip for gold
        'decimals': 2,
        'tolerance_pips': 5.0,
    },
    'XAG_USD': {
        'name': 'Silver (XAG/USD)',
        'pip_size': 0.001,      # $0.001 = 1 pip for silver
        'decimals': 3,
        'tolerance_pips': 10.0,
    },
}


def fetch_data(pair: str, timeframe: str, count: int = 100) -> pd.DataFrame:
    tf_map = {'D': '1d', 'H4': '4h', 'H1': '1h', 'M15': '15m', 'M5': '5m'}
    tf = tf_map.get(timeframe, timeframe)
    df = fetcher.fetch_latest(pair, tf, count)
    df = df.reset_index()
    df.columns = ['time', 'open', 'high', 'low', 'close', 'volume']
    return df


def find_swing_points(df, lookback=2):
    swing_highs, swing_lows = [], []
    for i in range(lookback, len(df) - lookback):
        if all(df['high'].iloc[i] > df['high'].iloc[i-j] and
               df['high'].iloc[i] > df['high'].iloc[i+j] for j in range(1, lookback+1)):
            swing_highs.append({'idx': i, 'price': df['high'].iloc[i]})
        if all(df['low'].iloc[i] < df['low'].iloc[i-j] and
               df['low'].iloc[i] < df['low'].iloc[i+j] for j in range(1, lookback+1)):
            swing_lows.append({'idx': i, 'price': df['low'].iloc[i]})
    return swing_highs, swing_lows


def find_fvgs(df):
    bullish_fvgs, bearish_fvgs = [], []
    for i in range(2, len(df)):
        c1_high = df['high'].iloc[i-2]
        c3_low = df['low'].iloc[i]
        if c3_low > c1_high:
            fvg = {'idx': i-1, 'top': c3_low, 'bottom': c1_high,
                    'fifty': (c3_low + c1_high) / 2, 'mitigated': False}
            for j in range(i+1, len(df)):
                if df['low'].iloc[j] <= fvg['fifty']:
                    fvg['mitigated'] = True; break
            if not fvg['mitigated']:
                bullish_fvgs.append(fvg)

        c1_low = df['low'].iloc[i-2]
        c3_high = df['high'].iloc[i]
        if c1_low > c3_high:
            fvg = {'idx': i-1, 'top': c1_low, 'bottom': c3_high,
                    'fifty': (c1_low + c3_high) / 2, 'mitigated': False}
            for j in range(i+1, len(df)):
                if df['high'].iloc[j] >= fvg['fifty']:
                    fvg['mitigated'] = True; break
            if not fvg['mitigated']:
                bearish_fvgs.append(fvg)
    return bullish_fvgs, bearish_fvgs


def find_equal_levels(df, pip_size, tolerance_pips=5.0):
    swing_highs, swing_lows = find_swing_points(df)
    equal_highs, equal_lows = [], []
    tol = tolerance_pips * pip_size
    for i, sh1 in enumerate(swing_highs):
        for sh2 in swing_highs[i+1:]:
            if abs(sh1['price'] - sh2['price']) <= tol:
                equal_highs.append({'price': (sh1['price']+sh2['price'])/2,
                                    'idx1': sh1['idx'], 'idx2': sh2['idx']})
                break
    for i, sl1 in enumerate(swing_lows):
        for sl2 in swing_lows[i+1:]:
            if abs(sl1['price'] - sl2['price']) <= tol:
                equal_lows.append({'price': (sl1['price']+sl2['price'])/2,
                                   'idx1': sl1['idx'], 'idx2': sl2['idx']})
                break
    return equal_highs, equal_lows


def find_order_blocks(df):
    bullish_obs, bearish_obs = [], []
    for i in range(3, len(df)):
        if df['close'].iloc[i-2] < df['open'].iloc[i-2]:
            if (df['close'].iloc[i-1] > df['high'].iloc[i-2] and
                    df['close'].iloc[i] > df['close'].iloc[i-1]):
                bullish_obs.append({'idx': i-2, 'high': df['high'].iloc[i-2],
                                    'low': df['low'].iloc[i-2],
                                    'fifty': (df['high'].iloc[i-2]+df['low'].iloc[i-2])/2})
        if df['close'].iloc[i-2] > df['open'].iloc[i-2]:
            if (df['close'].iloc[i-1] < df['low'].iloc[i-2] and
                    df['close'].iloc[i] < df['close'].iloc[i-1]):
                bearish_obs.append({'idx': i-2, 'high': df['high'].iloc[i-2],
                                    'low': df['low'].iloc[i-2],
                                    'fifty': (df['high'].iloc[i-2]+df['low'].iloc[i-2])/2})
    return bullish_obs, bearish_obs


def detect_market_structure(swing_highs, swing_lows):
    if len(swing_highs) < 2 or len(swing_lows) < 2:
        return "NEUTRAL", "⚪"
    hh = swing_highs[-1]['price'] > swing_highs[-2]['price']
    hl = swing_lows[-1]['price'] > swing_lows[-2]['price']
    lh = swing_highs[-1]['price'] < swing_highs[-2]['price']
    ll = swing_lows[-1]['price'] < swing_lows[-2]['price']
    if hh and hl: return "BULLISH", "🟢"
    elif lh and ll: return "BEARISH", "🔴"
    elif hh and ll: return "RANGING", "🟡"
    else: return "MIXED", "⚪"


def detect_mss(df, swing_highs, swing_lows):
    mss_events = []
    if len(swing_highs) >= 2:
        last_sh = swing_highs[-2]
        for i in range(last_sh['idx']+1, len(df)):
            if df['close'].iloc[i] > last_sh['price']:
                mss_events.append({'idx': i, 'price': last_sh['price'],
                                   'direction': 'BULLISH', 'label': 'MSS ↑'})
                break
    if len(swing_lows) >= 2:
        last_sl = swing_lows[-2]
        for i in range(last_sl['idx']+1, len(df)):
            if df['close'].iloc[i] < last_sl['price']:
                mss_events.append({'idx': i, 'price': last_sl['price'],
                                   'direction': 'BEARISH', 'label': 'MSS ↓'})
                break
    return mss_events


def plot_chart(df, tf, ax, decimals, pip_size, tolerance_pips):
    swing_highs, swing_lows = find_swing_points(df)
    bull_fvgs, bear_fvgs = find_fvgs(df)
    equal_highs, equal_lows = find_equal_levels(df, pip_size, tolerance_pips)
    bull_obs, bear_obs = find_order_blocks(df)
    mss_events = detect_mss(df, swing_highs, swing_lows)
    structure, emoji = detect_market_structure(swing_highs, swing_lows)

    fmt = f".{decimals}f"
    range_high = df['high'].max()
    range_low = df['low'].min()
    equilibrium = (range_high + range_low) / 2

    # Premium/Discount
    ax.axhspan(equilibrium, range_high, alpha=0.04, color='red')
    ax.axhspan(range_low, equilibrium, alpha=0.04, color='green')
    ax.axhline(y=equilibrium, color='yellow', linestyle='-', linewidth=1.5, alpha=0.6)
    ax.annotate(f"EQ: {equilibrium:{fmt}}", (3, equilibrium), fontsize=6, color='yellow', alpha=0.8)

    # Candlesticks
    for i in range(len(df)):
        color = '#00ff00' if df['close'].iloc[i] >= df['open'].iloc[i] else '#ff0000'
        ax.plot([i, i], [df['low'].iloc[i], df['high'].iloc[i]], color=color, linewidth=0.5)
        body_bottom = min(df['open'].iloc[i], df['close'].iloc[i])
        body_top = max(df['open'].iloc[i], df['close'].iloc[i])
        body_height = max(body_top - body_bottom, pip_size * 0.01)
        rect = mpatches.Rectangle((i-0.3, body_bottom), 0.6, body_height, linewidth=0, facecolor=color)
        ax.add_patch(rect)

    # Bullish FVGs
    for fvg in bull_fvgs[-3:]:
        rect = mpatches.Rectangle((fvg['idx'], fvg['bottom']),
                                   len(df)-fvg['idx'], fvg['top']-fvg['bottom'],
                                   linewidth=1, facecolor='#00ff00', alpha=0.12, edgecolor='#00ff00')
        ax.add_patch(rect)
        ax.axhline(y=fvg['fifty'], color='#00ff00', linestyle=':', linewidth=0.8, alpha=0.7)
        ax.annotate(f"BISI: {fvg['fifty']:{fmt}}", (len(df)-5, fvg['fifty']),
                     fontsize=5.5, color='#00ff00', ha='right')

    # Bearish FVGs
    for fvg in bear_fvgs[-3:]:
        rect = mpatches.Rectangle((fvg['idx'], fvg['bottom']),
                                   len(df)-fvg['idx'], fvg['top']-fvg['bottom'],
                                   linewidth=1, facecolor='#ff0000', alpha=0.12, edgecolor='#ff0000')
        ax.add_patch(rect)
        ax.axhline(y=fvg['fifty'], color='#ff0000', linestyle=':', linewidth=0.8, alpha=0.7)
        ax.annotate(f"SIBI: {fvg['fifty']:{fmt}}", (len(df)-5, fvg['fifty']),
                     fontsize=5.5, color='#ff0000', ha='right')

    # Order Blocks
    for ob in bull_obs[-2:]:
        rect = mpatches.Rectangle((ob['idx']-0.5, ob['low']),
                                   len(df)-ob['idx'], ob['high']-ob['low'],
                                   linewidth=2, facecolor='none', edgecolor='#00ffff', linestyle='--')
        ax.add_patch(rect)
        ax.annotate("B-OB", (ob['idx'], ob['low']), fontsize=6, color='#00ffff', fontweight='bold')
    for ob in bear_obs[-2:]:
        rect = mpatches.Rectangle((ob['idx']-0.5, ob['low']),
                                   len(df)-ob['idx'], ob['high']-ob['low'],
                                   linewidth=2, facecolor='none', edgecolor='#ff6600', linestyle='--')
        ax.add_patch(rect)
        ax.annotate("S-OB", (ob['idx'], ob['high']), fontsize=6, color='#ff6600', fontweight='bold')

    # Equal Highs / Lows
    for eh in equal_highs[-2:]:
        ax.axhline(y=eh['price'], color='#ff00ff', linestyle='--', linewidth=2, alpha=0.8)
        ax.annotate(f"EQH/BSL: {eh['price']:{fmt}}", (len(df)-3, eh['price']),
                     fontsize=6.5, color='#ff00ff', ha='right', fontweight='bold')
    for el in equal_lows[-2:]:
        ax.axhline(y=el['price'], color='#00ffff', linestyle='--', linewidth=2, alpha=0.8)
        ax.annotate(f"EQL/SSL: {el['price']:{fmt}}", (len(df)-3, el['price']),
                     fontsize=6.5, color='#00ffff', ha='right', fontweight='bold')

    # MSS
    for mss in mss_events[-2:]:
        mc = '#00ff00' if mss['direction'] == 'BULLISH' else '#ff0000'
        ax.axhline(y=mss['price'], color=mc, linestyle='-.', linewidth=1.5, alpha=0.6)
        ax.annotate(mss['label'], (mss['idx'], mss['price']), fontsize=7, color=mc,
                     fontweight='bold', bbox=dict(boxstyle='round,pad=0.2',
                     facecolor='#1a1a1a', edgecolor=mc, alpha=0.8))

    # Swing markers
    for sh in swing_highs[-5:]:
        ax.scatter(sh['idx'], sh['price'], marker='v', color='yellow', s=30, zorder=5)
    for sl in swing_lows[-5:]:
        ax.scatter(sl['idx'], sl['price'], marker='^', color='yellow', s=30, zorder=5)

    # Current price
    current = df['close'].iloc[-1]
    zone = "PREMIUM" if current > equilibrium else "DISCOUNT"
    zone_color = '#ff6666' if zone == "PREMIUM" else '#66ff66'
    ax.axhline(y=current, color='white', linestyle='-', linewidth=1.8, alpha=0.9)
    ax.annotate(f"NOW: {current:{fmt}}", (len(df)-1, current), fontsize=7, color='white',
                 fontweight='bold', ha='right',
                 bbox=dict(boxstyle='round', facecolor='#333', alpha=0.9))

    ax.set_title(f"{tf}: {emoji} {structure} | {zone}\n"
                 f"Bull FVG: {len(bull_fvgs)} | Bear FVG: {len(bear_fvgs)} | "
                 f"B-OB: {len(bull_obs)} | S-OB: {len(bear_obs)}",
                 fontsize=9, fontweight='bold', color=zone_color)
    ax.grid(True, alpha=0.12)
    ax.set_ylabel('Price', fontsize=7)

    tick_positions = range(0, len(df), max(1, len(df)//5))
    tick_labels = [df['time'].iloc[i].strftime('%m/%d %H:%M') for i in tick_positions]
    ax.set_xticks(list(tick_positions))
    ax.set_xticklabels(tick_labels, fontsize=5, rotation=20)

    return {
        'structure': structure, 'emoji': emoji, 'zone': zone,
        'current': current, 'equilibrium': equilibrium,
        'bull_fvgs': bull_fvgs, 'bear_fvgs': bear_fvgs,
        'bull_obs': bull_obs, 'bear_obs': bear_obs,
        'equal_highs': equal_highs, 'equal_lows': equal_lows,
        'mss_events': mss_events, 'swing_highs': swing_highs, 'swing_lows': swing_lows,
        'range_high': range_high, 'range_low': range_low,
    }


def print_thesis(pair_name, analysis, pip_size, decimals):
    fmt = f".{decimals}f"
    daily = analysis['DAILY']
    h4 = analysis['4-HOUR']
    h1 = analysis['1-HOUR']
    m15 = analysis['15-MIN']

    print(f"\n{'='*70}")
    print(f"  📋 VEX ICT TRADE THESIS — {pair_name}")
    print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*70}")

    print(f"\n{'─'*50}")
    print(f"  📊 HIGHER TIMEFRAME BIAS")
    print(f"{'─'*50}")
    print(f"  Daily Structure:  {daily['emoji']} {daily['structure']}")
    print(f"  Daily Zone:       {daily['zone']}")
    print(f"  Daily EQ:         {daily['equilibrium']:{fmt}}")
    print(f"  4H Structure:     {h4['emoji']} {h4['structure']}")
    print(f"  4H Zone:          {h4['zone']}")
    print(f"  4H EQ:            {h4['equilibrium']:{fmt}}")

    htf_bullish = daily['structure'] in ['BULLISH','MIXED'] and daily['zone'] == 'DISCOUNT'
    htf_bearish = daily['structure'] in ['BEARISH','MIXED'] and daily['zone'] == 'PREMIUM'

    if htf_bullish:
        overall_bias = "BULLISH 🟢"; bias_dir = "LONG"
    elif htf_bearish:
        overall_bias = "BEARISH 🔴"; bias_dir = "SHORT"
    elif h4['structure'] == 'BULLISH':
        overall_bias = "LEAN BULLISH 🟢"; bias_dir = "LONG"
    elif h4['structure'] == 'BEARISH':
        overall_bias = "LEAN BEARISH 🔴"; bias_dir = "SHORT"
    else:
        overall_bias = "NEUTRAL ⚪"; bias_dir = "WAIT"

    print(f"\n  ➤ OVERALL BIAS: {overall_bias}")

    print(f"\n{'─'*50}")
    print(f"  🎯 1-HOUR SETUP")
    print(f"{'─'*50}")
    print(f"  1H Structure:     {h1['emoji']} {h1['structure']}")
    print(f"  1H Zone:          {h1['zone']}")
    print(f"  Current Price:    {h1['current']:{fmt}}")

    if h1['mss_events']:
        m = h1['mss_events'][-1]
        print(f"  MSS Detected:     {m['label']} at {m['price']:{fmt}}")

    print(f"\n{'─'*50}")
    print(f"  📍 ENTRY ARRAYS")
    print(f"{'─'*50}")

    if bias_dir == "LONG":
        if h1['bull_fvgs']:
            print(f"\n  BULLISH ENTRIES:")
            for fvg in h1['bull_fvgs'][-3:]:
                dist = abs(h1['current'] - fvg['fifty']) / pip_size
                print(f"    → BISI FVG 50%: {fvg['fifty']:{fmt}} ({dist:.0f} pips away)")
        if h1['bull_obs']:
            print(f"  BULLISH OBs:")
            for ob in h1['bull_obs'][-2:]:
                print(f"    → B-OB: {ob['low']:{fmt}} - {ob['high']:{fmt}} (50%: {ob['fifty']:{fmt}})")
        print(f"\n  TARGETS:")
        if h1['equal_highs']:
            for eh in h1['equal_highs'][-2:]:
                dist = abs(eh['price'] - h1['current']) / pip_size
                print(f"    → BSL (EQH): {eh['price']:{fmt}} ({dist:.0f} pips)")
        print(f"    → Range High: {h1['range_high']:{fmt}}")
        if h1['swing_lows']:
            sl = h1['swing_lows'][-1]['price'] - 5*pip_size
            risk = abs(h1['current'] - sl) / pip_size
            print(f"\n  STOP: Below {sl:{fmt}} (~{risk:.0f} pips)")

    elif bias_dir == "SHORT":
        if h1['bear_fvgs']:
            print(f"\n  BEARISH ENTRIES:")
            for fvg in h1['bear_fvgs'][-3:]:
                dist = abs(h1['current'] - fvg['fifty']) / pip_size
                print(f"    → SIBI FVG 50%: {fvg['fifty']:{fmt}} ({dist:.0f} pips away)")
        if h1['bear_obs']:
            print(f"  BEARISH OBs:")
            for ob in h1['bear_obs'][-2:]:
                print(f"    → S-OB: {ob['low']:{fmt}} - {ob['high']:{fmt}} (50%: {ob['fifty']:{fmt}})")
        print(f"\n  TARGETS:")
        if h1['equal_lows']:
            for el in h1['equal_lows'][-2:]:
                dist = abs(el['price'] - h1['current']) / pip_size
                print(f"    → SSL (EQL): {el['price']:{fmt}} ({dist:.0f} pips)")
        print(f"    → Range Low: {h1['range_low']:{fmt}}")
        if h1['swing_highs']:
            sh = h1['swing_highs'][-1]['price'] + 5*pip_size
            risk = abs(sh - h1['current']) / pip_size
            print(f"\n  STOP: Above {sh:{fmt}} (~{risk:.0f} pips)")
    else:
        print(f"\n  ⚠️  NO CLEAR BIAS — STAY FLAT")

    # 15M confirmation
    print(f"\n{'─'*50}")
    print(f"  🔍 15-MIN CONFIRMATION")
    print(f"{'─'*50}")
    print(f"  15M Structure:    {m15['emoji']} {m15['structure']}")
    if m15['mss_events']:
        m = m15['mss_events'][-1]
        aligned = (bias_dir=="LONG" and m['direction']=="BULLISH") or \
                  (bias_dir=="SHORT" and m['direction']=="BEARISH")
        print(f"  15M MSS:          {m['label']} at {m['price']:{fmt}}")
        print(f"  {'✅ Confirms!' if aligned else '⚠️  Not aligned — wait'}")
    else:
        print(f"  ⚠️  No 15M MSS yet")

    # Trade plan
    print(f"\n{'='*70}")
    print(f"  📌 TRADE PLAN — {pair_name}")
    print(f"{'='*70}")
    print(f"  Bias:       {overall_bias}")
    print(f"  Direction:  {bias_dir}")

    if bias_dir == "LONG" and h1['bull_fvgs']:
        entry = h1['bull_fvgs'][-1]['fifty']
        target = h1['range_high']
        stop = h1['swing_lows'][-1]['price'] - 5*pip_size if h1['swing_lows'] else h1['range_low']
        reward = abs(target - entry) / pip_size
        risk = abs(entry - stop) / pip_size
        rr = reward / risk if risk > 0 else 0
        print(f"  Entry:      {entry:{fmt}} (BISI FVG 50%)")
        print(f"  Stop:       {stop:{fmt}}")
        print(f"  Target:     {target:{fmt}}")
        print(f"  R:R =       1:{rr:.1f}")
    elif bias_dir == "SHORT" and h1['bear_fvgs']:
        entry = h1['bear_fvgs'][-1]['fifty']
        target = h1['range_low']
        stop = h1['swing_highs'][-1]['price'] + 5*pip_size if h1['swing_highs'] else h1['range_high']
        reward = abs(entry - target) / pip_size
        risk = abs(stop - entry) / pip_size
        rr = reward / risk if risk > 0 else 0
        print(f"  Entry:      {entry:{fmt}} (SIBI FVG 50%)")
        print(f"  Stop:       {stop:{fmt}}")
        print(f"  Target:     {target:{fmt}}")
        print(f"  R:R =       1:{rr:.1f}")
    else:
        print(f"  ⚠️  No clean setup — PATIENCE")

    print(f"\n{'='*70}\n")


def run_markup(pair_key, metal_cfg):
    pair_name = metal_cfg['name']
    pip_size = metal_cfg['pip_size']
    decimals = metal_cfg['decimals']
    tol = metal_cfg['tolerance_pips']

    print(f"\n{'#'*70}")
    print(f"  VEX CHART MARKUP: {pair_name}")
    print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'#'*70}\n")

    timeframes = {
        'DAILY': ('D', 60),
        '4-HOUR': ('H4', 100),
        '1-HOUR': ('H1', 100),
        '15-MIN': ('M15', 100),
    }

    data = {}
    for label, (tf, count) in timeframes.items():
        print(f"  Fetching {label}...", end=" ")
        data[label] = fetch_data(pair_key, tf, count)
        print(f"✓ {len(data[label])} candles")

    fig, axes = plt.subplots(2, 2, figsize=(20, 13))
    fig.patch.set_facecolor('#0a0a0a')
    fig.suptitle(
        f"{pair_name} MULTI-TIMEFRAME ICT MARKUP — {datetime.now().strftime('%Y-%m-%d %H:%M')}\n"
        f"🟢 Green = Bullish FVG (BISI) | 🔴 Red = Bearish FVG (SIBI) | ⬜ Dotted = 50% CE\n"
        f"🟣 Magenta = EQH (BSL) | 🔵 Cyan = EQL (SSL) / B-OB | 🟠 Orange = S-OB | 🟡 Yellow = EQ",
        fontsize=11, fontweight='bold', color='white'
    )

    positions = [(0,0), (0,1), (1,0), (1,1)]
    labels_list = ['DAILY', '4-HOUR', '1-HOUR', '15-MIN']
    analysis = {}

    for i, label in enumerate(labels_list):
        row, col = positions[i]
        print(f"  Plotting {label}...", end=" ")
        analysis[label] = plot_chart(data[label], label, axes[row][col], decimals, pip_size, tol)
        print("✓")

    plt.tight_layout(rect=[0, 0, 1, 0.92])

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    safe_name = pair_key.replace('_', '')
    output_path = f"/Users/villain/Documents/transfer/ICT_WORK/ict_trainer/screenshots/{safe_name}_markup_{timestamp}.png"
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='#0a0a0a')
    print(f"\n✅ Chart saved: {output_path}")
    plt.close()

    subprocess.run(["open", output_path])
    print_thesis(pair_name, analysis, pip_size, decimals)

    return output_path, analysis


def main():
    print(f"\n{'='*70}")
    print(f"  VEX METALS MARKUP — XAU/USD & XAG/USD")
    print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Market: {'OPEN ✅' if datetime.now().weekday() < 5 else 'CLOSED 🔴'}")
    print(f"{'='*70}")

    for pair_key, cfg in METALS.items():
        run_markup(pair_key, cfg)


if __name__ == "__main__":
    main()
